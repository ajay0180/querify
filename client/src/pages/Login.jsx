...
export default function Login() {
  ...
  const { userDetails, userToken, updateUserDetails, setUserToken } =
    useUserStore((state) => ({
  ...
  return (
    <motion.div className="bg-[#191919] h-[100vh]  w-full flex flex-row ">
      {/* left part */}
      <motion.div
      ...
      >
        <form
          onSubmit={formSubmit}
          className="w-full  p-[20px] flex flex-col justify-center items-center my-auto "
        >
          {/* heading */}
          <div className="flex flex-col justify-center items-center gap-4">
            <h2 className="heading">Querify</h2>
            <p className=" text-[0.8rem] sm:[1rem] md:text-[1.3rem] font-extralight text-center text-[#5E5E5E]">
              Hello, who's this ?{" "}
            </p>
          </div>
            ...
            <div className="flex gap-5  xxs:min-w-[300px] xxs:max-w-[300px] min-w-[250px] max-w-[250px] items-center justify-center border-[1px] border-black px-2 rounded-md">
              ...
              {isShowPassword ? (
                <FaEye
                  ...
                  className="text-[#c3c8ce] cursor-pointer text-md"
                />
              ) : (
                <FaEyeSlash
                  onClick={() => setIsShowPassword((prev) => !prev)}
            ...
            <div className="xxs:min-w-[300px] xxs:max-w-[300px] min-w-[250px] max-w-[250px] mt-4 ">
              <div className="flex gap-4 items-baseline">
                ...
                <p className="text-[0.88rem]">
                  ...
                  <Link className="underline">agree to abide by the terms</Link>{" "}
                  set forth by Querify.
                </p>
              </div>
            </div>
      ...
      >
        <div className="absolute top-3 right-0 flex items-center gap-5 mr-4">
          ...
          <Link
            to="/signup"
            className="text-white font-extralight border-[1px] border-white px-2 rounded-md"
          >
            {" "}
            Signup{" "}
          </Link>
        ...
        <div className=" text-center">
          <p className="text-white text-[2.3rem] ">
            Login <br /> and come on in
          </p>
        </div>

        <div className="xl:w-[350px] w-[300px] xl:h-[350px] h-[300px] mt-3">
          <img src={signupImage} alt="signupImage" className="object-contain" />
        </div>
