  ...
  const fetchResponses = async () => {
    try {
      ...
      const result = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/getAllResponse`,
        { formId },
        {
          headers: {
    ...
    const freqConstruct = () => {
      ...
      radioQues.forEach((que) => {
        ...
        responses.forEach((resp) => {
          obj[resp.data[`q${que}`]] = obj[resp.data[`q${que}`]]
            ? obj[resp.data[`q${que}`]] + 1
            : 1;
        });
      ...
      for (let que in data) {
        ...
        if (queOptions?.length > arr.length) {
          for (var i = 1; i <= queOptions.length; i++) {
            // put the missing options
  ...
  return (
    <div className="w-screen  min-h-screen h-max relative bg-[#fafafa]">
        ...
        <div className="h-[54px] w-[70%] flex items-center gap-5">
          <div
            className={`relative flex items-center h-full ${
              selected === "insights" && "border-b-[2px] border-black"
            }`}
            onClick={() => setSelected("insights")}
          >
            <p
              className={`text-[13px]  cursor-pointer tracking-widest  ${
                selected === "insights"
                  ? "text-[#00000]"
                  : "text-[#9b9a9a] hover:text-[#000000ce] transition-all duration-100"
              }`}
            >
              INSIGHTS
            </p>
          ...
          <div
            className={`relative flex items-center h-full ${
              selected === "visual" && "border-b-[2px] border-black"
            }`}
          ...
          <div
            className={`relative flex items-center h-full ${
              selected === "responses" && "border-b-[2px] border-black"
            }`}
      ...
      {loading ? (
        ...
        <div
          className="sm:w-[70%] w-[90%] mx-auto   h-max  py-[30px] top-[104px] relative "
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="mt-[25px] flex flex-col gap-5 w-full">
                  <p className="text-[24px] tracking-wide font-light">
                    {" "}
                    Response Summary{" "}
                  </p>
                  ...
                  <div className="flex justify-center gap-3 w-full">
                    <div
                      className="flex flex-col h-[352px] c1:min-w-[704px] w-full  bg-white rouned-md "
                      style={{
                        boxShadow:
                          "0 1px 2px 0 rgba(0, 0, 0, 0.04), 0 2px 8px 0 rgba(0, 0, 0, 0.04)",
                      }}
                    >
                      <div className="w-full h-[98px] pt-[24px] px-[32px] flex flex-col gap-2">
                        <div className="flex items-center  gap-4 bg-[#69B5FC] w-max px-1 py-[2px] rounded-md">
                          <svg
                            className="SVGInline-svg fill-black text-lg"
                        ...
                        <p className="text-[12px] text-[#807e7e] pl-[50px]">
                          {responses.filter((resp) => resp.name).length} out of{" "}
                          {responses.length} people provided their personal
                          details
                        </p>
                      </div>
                      ...
                      <div className="w-full c1:px-[96px] px-[10px] pb-[56px] ">
                        <div className="w-full max-h-[200px] mt-[10px] overflow-y-scroll">
                          {responses
                            .filter((resp) => resp.name)
                            .map((response) => (
                              <div className=" w-full flex flex-col justify-center h-[180px] max-h-[200px] p-[30px] border-b">
                                <div className="flex gap-3 mb-2">
                                  <p className="p-0 min-w-[15%] overflow-y-hidden whitespace-nowrap text-[13px] text-[#807e7e]">
                                    {" "}
                                    name{" "}
                                  </p>
                                  <p className="p-0 text-[14px]">
                                    {" "}
                                    {response.name}
                                  </p>
                                </div>
                    ...
                    <div className="lg:flex hidden flex-col gap-5 min-w-[256px] max-w-[256px]">
                      <div className="p-[20px] w-full bg-white rounded-md flex gap-5">
                        <div>
                          <svg height="36" width="24" viewBox="0 0 212 283">
                            <g fill="none" fill-rule="evenodd">
                              <path
                                d="M17 0h124l71 71v195c0 9.389-7.611 17-17 17H17c-9.389 0-17-7.611-17-17V17C0 7.611 7.611 0 17 0z"
                                fill="#13A562"
                              ></path>
                              <g fill="#F1F1F1">
                                <path d="M55 158h103v12H55z"></path>
                                <path d="M101 138h12v83h-12z"></path>
                                <path d="M55 188h103v12H55z"></path>
                                <path
                                  d="M60 142v75h93v-75H60zm-12-12h117v99H48v-99z"
                                  fill-rule="nonzero"
                                ></path>
                              </g>
                              <path
                                d="M146 66l66 67V70.992z"
                                fill="#049052"
                              ></path>
                              <path
                                d="M141 0l71 71h-54c-9.389 0-17-7.611-17-17V0z"
                                fill="#8BD1AF"
                              ></path>
                            </g>
                          </svg>
                        ...
                        <div className="flex flex-col">
                          ...
                          <p className="text-[12px] ">
                            {form.spreadsheetUrl ? (
                              <p className="flex gap-3 mt-7 items-center text-[16px]">
                                <TiTick className="fill-green-500 text-[24px]" />{" "}
                                Integrated
                              </p>
                            ) : (
                              <p>
                                Connect and send your data straight to a
                                spreadsheet.
                              </p>
                            )}
                          </p>
                      ...
                      <div className="p-[20px] w-full bg-[#f0f0f0] rounded-md">
                        <p className="text-[14px]">
                          Share your results with anyone. Your report
                          automatically updates as new answers come in.
                        </p>
                      </div>
                      <div className="px-[50px] py-1 rounded-md flex items-center justify-center text-white bg-black cursor-pointer hover:bg-zinc-600 transition-all duration-200 text-[14px]">
                        Generate a report
                      </div>
                    </div>
                  </div>
