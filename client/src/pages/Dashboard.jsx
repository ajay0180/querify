...
export default function Dashboard() {
  ...
  const getForms = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/getAllForms`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div className="w-screen min-h-screen h-max  bg-[#fafafa] pt-[54px] relative">
      ...
      <div
        className={`${
          activeSidebar ? "ml-[256px]" : ""
        } flex flex-col p-[20px] pt-0  `}
      >
        {/* Navbar */}

        <div
          className={`fixed ${
            activeSidebar ? "left-[256px]" : "left-0"
          } top-[50px] z-[10] bg-[#fafafa] h-[80px]   flex items-center px-[20px]  border-b  justify-between`}
          style={
            activeSidebar
        ...
        <div className="flex flex-wrap gap-4 mt-[120px] ">
          {loading ? (
            ...
            formsData.map((form) => <FormCard form={form} key={form._id} />)
          )}
        </div>
      </div>
    </div>
